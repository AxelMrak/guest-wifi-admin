...
// Smoke test: confirmar que rkey.uci responde y leer el estado actual
try {
  const res = await routerService.callService("rkey.uci", "get", { config: "wificfg" });
  const values = (res as { result?: [number, { values?: Record<string, Record<string, string>> }] }).result?.[1]?.values;
  if (values) {
    const sections = Object.keys(values).filter((k) => k !== "global");
    console.log(`[diag] UBUS OK — secciones wificfg: ${sections.join(", ")}`);
  } else {
    console.log("[diag] UBUS no devolvió valores esperados");
  }
} catch (err) {
  console.log(`[diag] UBUS smoke test falló: ${(err as Error).message}`);
}
